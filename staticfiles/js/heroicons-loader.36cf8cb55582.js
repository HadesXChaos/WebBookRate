/**
 * Heroicons Loader - Load Heroicons SVG icons dynamically
 * Since Heroicons React requires build step, we use SVG directly
 */

window.Heroicons = {
  /**
   * Render Heroicon SVG
   * @param {string} iconName - Name of the icon (e.g., 'heart', 'book', 'user')
   * @param {string} variant - 'outline' or 'solid' (default: 'outline')
   * @param {object} attrs - Additional attributes for SVG
   */
  render: function(iconName, variant = 'outline', attrs = {}) {
    const icons = this.getIcon(iconName, variant);
    if (!icons) {
    ...
    const defaultAttrs = {
      ...
      viewBox: '0 0 24 24',
      stroke: variant === 'outline' ? 'currentColor' : null,
      'stroke-width': variant === 'outline' ? '1.5' : null,
      class: attrs.class || '',
      ...attrs
    };
    
    if (variant === 'solid') {
      delete defaultAttrs.stroke;
      delete defaultAttrs['stroke-width'];
    } else {
      delete defaultAttrs.fill;
    }
    ...
    return `<svg ${attrString}>${icons.path}</svg>`;
  },
  
  /**
   * Get icon path data
   */
  getIcon: function(iconName, variant = 'outline') {
    // Common Heroicons - you can extend this
    const iconMap = {
      outline: {
      ...
      solid: {
        ...
        book: '<path d="M11.25 4.533A9 9 0 1011.25 4.533z" />',
        user: '<path fill-rule="evenodd" d="M7.5 6a4.5 4.5 0 119 0 4.5 4.5 0 01-9 0zM3.751 20.105a8.25 8.25 0 0116.498 0 .75.75 0 01-.437.695A18.683 18.683 0 0112 22.5c-2.786 0-5.433-.608-7.812-1.7a.75.75 0 01-.437-.695z" clip-rule="evenodd" />',
    ...
    return iconMap[variant] && iconMap[variant][iconName] 
      ? { path: iconMap[variant][iconName] }
      : null;
  }
};
